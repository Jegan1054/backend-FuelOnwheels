...
if ($stmt->fetch()) {
    ...
    echo json_encode(['error' => 'Shop already registered']);
    exit;
}

$stmt = $pdo->prepare("INSERT INTO shops (user_id, name, type, latitude, longitude, radius) VALUES (?, ?, 'mechanic', ?, ?, ?)");
if (!$stmt->execute([$user['id'], $name, $latitude, $longitude, $radius])) {
    http_response_code(500);
    echo json_encode(['error' => 'Failed to register shop']);
    exit;
}
